...
DefaultParameters <- c(Delta = 2, #De Roos 2008 Delta = 1
                       
                       Rmax = 3, #De Roos 2008 Rmax = 2
                       A_hat = .5, #guess                                                              
                       Temp = 293.15,  #20 C is 293.15 K
                       E_mu = .2, #guess
                       E_M = .2, #guess
                       E_I = .2, #guess
                       E_delta = .2, #guess
                       ...
                       sigma = .5, # (Peters 1983) (Yodzis and Innes 1992) (De Roos et al. 2008)
                       T_0 = 295.15,
                       ...
                       rho1 = .0123,
                       rho2 = .77,
                       mj = .0154*150^2.71, #Mass when zoop changes from juvenile to adult Length from Christine T. Baier, Jeffrey M. Napp 2003.  a and b parameters are from calanus finmarchius
                       mb = .01*(.0154*150^2.71), #Mass of zoop at birth taken from value of z from de Roos et al 2008
                       z = (.01*(.0154*150^2.71))/(.0154*150^2.71) #Z is used for the maturation rate using the equation from the proposal. z = ratio of juvenile to adult individual mass 
) 
...
#Negative_to_zero(sigma*( ( ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R) /(1+( ( ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)) / ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*epsilon1*Mass^(epsilon2)) )) - (exp(E_M*(Temp-T_0)/(k*Temp*T_0))*rho1*Mass^rho2) )


LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  
  # rI = exp(E_I*(Temp-T_0)/k*Temp*T_0)
  # rM = exp(E_M*(Temp-T_0)/k*Temp*T_0)
  # ai = rI*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)
  # ni = ai*R #Encounter rate
  # Imax = rI*epsilon1*Mass^(epsilon2)
  # I = ni/(1+(ni/Imax)) #ingestion 
  # Metabolic_rate = rM*rho1*Mass^rho2
  # I = ( ( ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)) *R) /(1+( ( ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)) *R)) / ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*epsilon1*Mass^(epsilon2)) ))
  with(as.list(c(E, pars, istate)),{
    list(
      ...
      development =  c(      1.0, Negative_to_zero(sigma*( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R) /(1+( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)) / ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*epsilon1*Mass^(epsilon2)) )) - (exp(E_M*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*rho1*Mass^rho2) )  #sigma times ingestion              
                                          
                             #Ignores the maturation rate equation.  Not entirely sure how that works.
                             
      ), 
      
      #print( ((sigma*(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/(1+((((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*epsilon1*Mass^(epsilon2))))) - ((exp(E_M*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*rho1*Mass^rho2) )),
      #Net production currently negative net production is allowed meaning there is biomass loss from starvation
      
      fecundity   = switch(lifestage, 0, Negative_to_zero(sigma*( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R) /(1+( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)) / ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*epsilon1*Mass^(epsilon2)) )) - (exp(E_M*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*rho1*Mass^rho2) )/mb ),
      
      mortality   = switch(lifestage, exp(E_mu*(Temp-T_0)/((E_mu/Temp)*Temp*T_0))*phi1*Mass^phi2, exp(E_mu*(Temp-T_0)/((E_mu/Temp)*Temp*T_0))*phi1*Mass^phi2  ),
      
      impact      = switch(lifestage, c(( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)) *R) /(1+( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)) *R)) / ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*epsilon1*Mass^(epsilon2)) )), Mass, 0), #juveniles
                           
                           c(( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)) *R) /(1+( ( ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha)) *R)) / ((exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0)))*epsilon1*Mass^(epsilon2)) )), 0, Mass)) #adults  )  
...
#exp(E_delta*(Temp-T_0)/((E_delta/Temp)*Temp*T_0))
