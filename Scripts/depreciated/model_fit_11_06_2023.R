  ...
  
{
  ...
  DF3_J = DF3[which(DF3[,"Stage"] == "Juvenile"),]
  DF3_A = DF3[which(DF3[,"Stage"] == "Adult"),]
...
Delta_Vector = seq(.01,.05,.01) 

Rmax_Vector =  2000 #seq(35,50,1) #2000
#A_Hat_Vector = seq(0.035,.065,.01) 
A_Hat_Vector = 0.096

alpha_Vector = seq(0.70,0.75,0.01)

Mopt_Vector = seq(50,150,20)
...
Obs_A[,"value"] = Obs_A[,"value"] + 0.00001 #Add constant to deal with zeros
Obs_J[,"value"] = Obs_J[,"value"] + 0.00001
Pred_A[,"value"] = Pred_A[,"value"] + 0.00001
Pred_J[,"value"] = Pred_J[,"value"] + 0.00001
...
#Imax Temp independent
 ...
 # 0.7
 # 2.248442
 plot(results_ordered$V7~results_ordered$alpha)
