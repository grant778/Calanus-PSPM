...
DefaultParameters <- c(Rho =  .5, #.1, #per day.  Range of between approximately .1 and 3 from Marañón et al. 2014.  They found no relationship between phytoplankton turnover rate and temperature  
                       ...
                       cM = 0.0,# Jan tests the Roach model with values of -.02, 0, and .02 "positive c-values benefit smaller over larger individuals in warmer environments"
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    n = exp(E_I*(Temp-t0)/((k)*Temp*t0))*(A_hat*((Size/Mopt)*exp(1-Size/Mopt))^(alpha_prime+cI*(Temp-t0)))*R #For both life stages n = ai*R
    #Should max attack rate A_hat be temp dependent?
    Imax = epsilon1*Size^(epsilon2) #based on dry weight for zooplankton and grams of Carbon for phytoplankton
