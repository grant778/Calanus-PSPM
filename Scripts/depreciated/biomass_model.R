...
#
PSPMdimensions <- c(PopulationNr = 1, IStateDimension = 2, LifeHistoryStages = 2, ImpactDimension = 4)
...
NumericalOptions <- c(MIN_SURVIVAL  = 1.0E-9,                                       # Survival at which individual is considered dead
                      ...
                      ALLOWNEGATIVE = 0,                                            # Allow negative population density?
...
EnvironmentState <- c(R = "GENERALODE") ## When we have indirect density dependence (i.e. via resource competition), this is always an ODE. When it is direct, it's always an integral!
...
DefaultParameters <- c(Rho = 0.1, Rmax = 0.01, Sb = 0.5, Sm = 1.0, Mc = 1.0, Hc = 2.0, Q = 0.5, Sigma = 0.5,
Tc = 0.1, Mu = 0.02)
...
StateAtBirth <- function(E, pars)
{
 with(as.list(c(E, pars)),{
    ...
    # 1: age (initial value 0); 2: length (initial value equal to parameter Sb)
    c(Age = 0.0, Size = Sb)
   })
}
...
LifeStageEndings <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
        maturation  = switch(lifestage, Size - Sm, -1) ## When Size-Sm = 0 marks the end of the life stage
      })
}
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    list(
      development = switch(lifestage, c(1.0,Sigma*(Mc*Size*R/(Hc + R)) - Tc*Size),  c(1.0,0.0)),
      
      fecundity   = switch(lifestage, 0.0, (Sigma*(Q*Mc*Size*R/(Hc + R)) - Tc*Size)/Sb),
      
      mortality   = Mu,
      
      impact      = switch(lifestage, c(Mc*Size*R/(Hc + R), Size, 0, Size), c(Q*Mc*Size*R/(Hc + R), 0, Size, Size))
    )
  })
}
## Impact: models are mass-specific, impact is a way to keep track on individual-level rates
## We can switch in two ways: either a full vector (as is done here) or levels within vector
## In this case we follow four impact variables: 1) ingestion 2) juvenile (hence 0 in second, adult life stage) 3) adult (hence 0 in juvenile) 4) size + size gives J+A
## NOTE: impact is on individual level and the model does integration to get population level. Without impact you only get population level

## The way the switch-function works in these cases is: 
## If you are in the first stage you contribute to 1 to the first impact and 0 to the second, and if you are in the second it's the other way around!
...
EnvEqui <- function(I, E, pars) {
 with(as.list(c(E, pars)),{
    Rho*(Rmax - R) - I[1] ## This is R, not C, hence [index = 1] I[1] Because ingestion is the first impact. It gets scales to population level in the model. Semi-chemostat dynamics
   })
}
