...
DefaultParameters <- c(Delta = .1, #De Roos 2008 Delta = 1
                       ...
                       rho1 = .0123,
                       rho2 = .77,
                       mj = 12154.25, #.0154*150^2.71, #Mass when zoop changes from juvenile to adult Length from Christine T. Baier, Jeffrey M. Napp 2003.  a and b parameters are from calanus finmarchius
                       mb = 121, #.01*(.0154*150^2.71), #Mass of zoop at birth taken from value of z from de Roos et al 2008
                       z = (121)/(12154.25))
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    n = exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R #For both life stages n = ai*R
    
    Imax = exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*epsilon1*Mass^(epsilon2)
    
    Metabolic_rate = exp(E_M*(Temp-T_0)/((E_M/Temp)*Temp*T_0))*rho1*Mass^rho2
    ...
    netproduction = Negative_to_zero(netproduction)
    
   
    
    kappa         = switch(lifestage, 1.0, 0)
    
    kappa_2         = switch(lifestage, 0, 1.0)
    ...
    list(
      ...
      development = c(1.0, (kappa*(netproduction))),
      
      fecundity   = switch(lifestage, 0, (kappa_2)*((netproduction/mb))),
