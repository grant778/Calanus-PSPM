...
#
PSPMdimensions <- c(PopulationNr = 1, IStateDimension = 2, LifeHistoryStages = 2, ImpactDimension = 5)
...
DefaultParameters <- c(Rho =  .25, #.1, #per day.  Range of between approximately .1 and 3 from Marañón et al. 2014.  They found no relationship between phytoplankton turnover rate and temperature  
                       ...
                       omega =  0.35063579, #size exponent ingestion, Saiz calbet 2007
                       beta = 0.88576314, #Resource exponent ingestion, Said Calbet 2007
                       theta =  0.01602631, #temperature coefficient ingestion, Saiz Calbet 2007
                       gamma =  -8.95972091, #intercept ingestion, Saiz Calbet 2007
...
LifeStageEndings <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    maturation  = switch(lifestage, Size - mj, -1) 
  })
}
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    list(
      ...
      impact      = switch(lifestage, c(Ingest, Size, 0, netproduction, 0), c(Ingest, 0, Size, 0, netproduction))
    )
  })
}
