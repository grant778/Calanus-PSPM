...
EQ_Function = function(Delta, alpha, Mopt)
  
{
  ...
  return(log(1000*DF_Summer_Observed_Predicted[which(DF_Summer_Observed_Predicted[,"Data_Type"] == "Predicted"),"value"])) #thousand µg
  
}

#t = EQ_Function(.01, .8, 47)
  
Nt = log(DF3[which(DF3[,"name"] == "Spring"),"value"]*1000)
CVt = sd(Nt,na.rm = TRUE)/mean(Nt, na.rm = TRUE)

minimize.NLL = function(par,Nt,CVt)
  
{
 ...
 Mopt = par[3]
  ...
  Ntpred <- EQ_Function(Delta, alpha, Mopt)

  NLL <- (log(Nt)-log(Ntpred))^2/(2*(CVt)^2)
...
#NLL = minimize.NLL(par,Nt,CVt)

z = optim(par=c(.1,.75,439.5),fn=minimize.NLL, method="Nelder-Mead",Nt = Nt ,CVt = CV, control = list(trace = TRUE))
