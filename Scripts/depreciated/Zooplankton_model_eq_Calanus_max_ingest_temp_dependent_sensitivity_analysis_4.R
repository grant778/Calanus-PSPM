...
DefaultParameters <- c(Delta = Delta, #turnover rate is 1 divided by the per capita growth rate
                       ...
                       epsilon1 = ,
                       epsilon2 = , #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species. 15 C.
                       epsilon3 = , #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species.  15 C.
                       epsilon4 = ,  #Approximated from saiz and calbert 2007
                       t0_epsilon = 288.15,
...
output1_1_non_trivial_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
                                      ...
                                      stepsize = .75, parbnds = c(1, output1_1_A_hat_.096$bifpoints[1], 2000), parameters = Modified_Parameters_A_hat_.096, 
...
Delta_Vector = seq(.05,.1,.01)
Rmax_Vector =  seq(700,1400,100)
#A_Hat_Vector = seq(0.035,.065,.01) 
A_Hat_Vector = seq(.09,.1,.001)
alpha_vector = .75
...
which(rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J)) <= 11)
results = data.frame(cbind(Delta_J,Rmax_J,A_Hat_J,rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J))))
Delta_J[10]
Rmax_J[10]
A_Hat_J[10]
# min(sensitivity_array[,1]) # 2,1 .6 and 2000
