...
DefaultParameters <- c(Delta =  1, #turnover rate is 1 divided by the per capita growth rate
                       ...
                       sigma = 0.67 , #0.6 (Kiørboe, 2008.) Converts ingested energy to biomass
                       #0.66 works well
                       Mopt = 250, #???????????
...
for(i in 1:nrow(DF3))
  {
    if(DF3[i,"Stage"] == "Juvenile")
      
    {
      DF3[i,"Stage"] = "J"
    }
    
    if(DF3[i,"Stage"] == "Adult")
  ...
  DF3[,"Data_Type"] = rep("Observed", nrow(DF3))
  
  DF_Observed_Predicted = rbind(DF3,Year_to_temp_DF)
  DF_Observed_Predicted[,2] = as.factor(DF_Observed_Predicted[,2])
  DF_Observed_Predicted[,5] = as.factor(DF_Observed_Predicted[,5])
  ...
  predicted_df = DF_Observed_Predicted[which(DF_Observed_Predicted[,"Data_Type"] == "Observed"),]
  ggplot(data =  predicted_df, aes(x =  Temperature, y = (value), color = Data_Type))+
    ...
    labs(x = "Temperature (K)", y ="Biomass Density (µg/L)")
  
    
  ggplot(data =  DF_Observed_Predicted, aes(x = Year, y = (value), color = Data_Type))+
    facet_wrap(~Stage, scale = "free", labeller = labeller_A_J)+
    theme_classic()+
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
    ...
    geom_line()+
    guides(color=guide_legend(title = "Data Type"))+
    labs(x = "Year", y ="Biomass Density (µg/L)")

  
  ggplot(data = DF_Observed_Predicted, aes(x = log(value), color = Data_Type, fill = Data_Type)) +
    facet_wrap(~Stage, scale = "free", labeller = labeller_A_J)+
