...
output1_1_non_trivial_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
                                      ...
                                      stepsize = .1, parbnds = c(1, output1_1_A_hat_.096$bifpoints[1], 30000), parameters = Modified_Parameters_A_hat_.096, 
...
start_.096 = mean(which(t1[,"Rmax"] < (Modified_Parameters_A_hat_.096[2]+100) & t1[,"Rmax"] > (Modified_Parameters_A_hat_.096[2]-100)))
...
output1_1_non_trivial_varying_temperature_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
 ...
 A_A_hat_Net_Production = Net_Production_A_hat_.096_A
  ...
  df4 <- data.frame(
    ...
    ggplot(df4, aes(x = R, y= value, color = name)) + 
...
Rmax_Vector = seq(300, 3100, 100)
#Delta_Vector = seq(.1,3.1,.5)

sensitivity_array = array(dim = c(length(Rmax_Vector), 2) )
Pred_J = array(dim = c(23, length(Rmax_Vector)))
Obs_J =  array(dim = c(23, length(Rmax_Vector)))
Pred_A =  array(dim = c(21, length(Rmax_Vector)))
Obs_A =  array(dim = c(21, length(Rmax_Vector)))
...
for(i in 1:length(Rmax_Vector))
{
    ...
    output = EQ_Function(Rmax_Vector[i])
