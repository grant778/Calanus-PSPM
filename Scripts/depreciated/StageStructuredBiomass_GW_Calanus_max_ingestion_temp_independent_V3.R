...
DefaultParameters <- c(Rho =  .5, #.1, #per day.  Range of between approximately .1 and 3 from Marañón et al. 2014.  They found no relationship between phytoplankton turnover rate and temperature  
                       Rmax = 1000, #Rmax is a density micrograms of carbon per liter.  This means all other densities including copepod densities are micrograms per liter. Approximately 2000 from Putland and Iverson 2007
                       
                       A_hat = 0.096, # .096 liters per day filtering rate AKA volume swept clear via frost 1972.  This value is actually nonlinear.  Currently using max value.  Units should be liters per day.
                                      # Neocalanus plumchrus is closer in size to C. marshallae at 567 µg.  Dagg and Wyman (1983) found a range of clearance rates between .0336 1.3344 L/day
                       ...
                       cM = 0,# Jan tests the Roach model with values of -.02, 0, and .02 
                       ...
                       alpha = 0.02162946, #Scalar. Saiz Calbert 2007 Calanoid copepod Data restricted to temps between 10 and 15 C.  Mean temp 286.953 K
                       alpha2 = 0.2492352, #Size exponent. Saiz Calbert 2007 Calanoid copepod Data restricted to temps between 10 and 15 C.  Mean temp 286.953 K
                       alpha3 = 0.8211947, #Resource exponent. Saiz Calbert 2007 Calanoid copepod Data restricted to temps between 10 and 15 C.  Mean temp 286.953 K
                       t0_alpha = 286.953,
                       epsilon1 = 1.252323, #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species.  Saiz and calbert found maximum ingestion rate was temp independent.
                       ...
                       #t0_ingestion = 286.953,
                       phi1 = 0.4025242, #For mortality rate per day.  Approximated from  Hirst and Kiørboe 2002 calculated at 15 C.  Dry weight
                       phi2 = -0.0085, #For mortality rate per day.  Approximated from  Hirst and Kiørboe 2002 calculated at 15 C. Dry Weight.  Might need to change my reference temp?
                       t0_phi = 288.15,
                       ...
                       t0_rho = 275.15, #micro grams per day, from Ikeda_2007.  Dry weight at 2 C
                       ...
                       mj = 2*3.5^3.94,   # ug (micrograms) (Petersen, 1986) pg 66
                       
                       z = 0.002694034 #juvenile to adult ratio 
                       )
                        #12.0 and 7.1 were percent of carbon found in dry weight from phytoplankton sample according to Curl Jr. H 19
                       #Mc = 1.0, Hc = 3.0, q = 0.5, Tc = 0.1, Muc = 0.015, Muj = 0.0, Mua = 0.0)
...
LifeStageEndings <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    maturation  = switch(lifestage, Size - 1.0, -1) #Scaled sizes so saying maturity is at size 1
  })
}
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    #Ingest = (Size^(omega)*R^(beta)*exp(theta*Temp)*exp(gamma)) #in units of micro grams of carbon per day.  Negative temperature dependence
   
    Ingest = exp(E_I*(Temp-t0_alpha)/((k)*Temp*t0_alpha))*(alpha*(Size)^(alpha2+cI*(Temp-t0_alpha))*R^(alpha3+cI*(Temp-t0_alpha))) #For both life stages n = ai*R.  Positive temperature dependence
    ...
    netproduction = (sigma*(Ingest))/.455 - Metabolic_rate*.014196*(1/21)*(1000/1) #need to convert ingestion parameter from micrograms of carbon to micrograms of dry weight of zooplankton with conversion factor of .455 (Uye 1982)
    
    #21 J/mg dry mass for Daphnia (Richman 1958, Cummins and Wuycheck 1971)
  ...
  # 1 J uses 0.002304147 mmol of Oxygen
    ...
    list(
      ...
      mortality   = switch(lifestage, exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^phi2, exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^phi2),
...
EnvEqui <- function(I, E, pars) {
  with(as.list(c(E, pars)),{
    Rho*(Rmax - R) - I[1] #Discuss temp dependence of delta
  })
}
