...
#
PSPMdimensions <- c(PopulationNr = 1, IStateDimension = 2, LifeHistoryStages = 8, ImpactDimension = 17)
...
StateAtBirth <- function(E, pars)
{
  with(as.list(c(E, pars)),{
    ...
    c(Age = 0.0, Size = Egg) #E/C6 = z = #juvenile to adult ratio
  })
}
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    list(
      ...
      fecundity   = switch(lifestage, 0,0,0,0,0,0,0,netproduction/Egg),
      
      mortality   = switch(lifestage, exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)), 
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)),
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)),
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)),
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)),
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)),
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)),
                           exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi))),  #Added temperature dependence to allometric exponent (Jan's paper doesn't include this)
      
      impact      = switch(lifestage, c(Ingest, Size, 0, 0, 0, 0, 0, 0, 0, netproduction, 0, 0, 0, 0, 0, 0, 0), 
                                      c(Ingest, 0, Size, 0, 0, 0, 0, 0, 0, 0, netproduction, 0, 0, 0, 0, 0, 0),
                                      c(Ingest, 0, 0, Size ,0, 0, 0, 0, 0, 0, 0, netproduction, 0, 0, 0, 0, 0),
                                      c(Ingest, 0, 0, 0, Size, 0, 0, 0, 0, 0, 0, 0, netproduction, 0, 0, 0, 0),
                                      c(Ingest, 0, 0, 0, 0, Size ,0, 0, 0, 0, 0, 0, 0, netproduction, 0, 0, 0),
                                      c(Ingest, 0, 0, 0, 0, 0, Size, 0, 0, 0, 0, 0, 0, 0, netproduction, 0, 0),
                                      c(Ingest, 0, 0, 0, 0, 0, 0, Size, 0, 0, 0, 0, 0, 0, 0, netproduction, 0),
                                      c(Ingest, 0, 0, 0, 0, 0, 0, 0, Size, 0, 0, 0, 0, 0, 0, 0, netproduction)
                                      )
    )
  })
}
