...
DefaultParameters <- c(Delta =  1, #turnover rate is 1 divided by the per capita growth rate
                       ...
                       A_hat = 1.096, #0.096 liters per day filtering rate AKA volume swept clear via frost 1972. Units should be liters per day.
                       ...
                       Im = 17.74286, #mean Imax from size and calbet max ingestion data restricted to 15 C
                       t0_Im = 288.15,
                       ...
                       epsilon1 = exp(-8.95972091),
                       epsilon2 = 0.35063579, #from Saiz and Calbet max ingestion data at 15 C
                       epsilon3 = 0.88576314,
                       epsilon4 = 0.01602631 ,
                       t0_epsilon = 288.15,
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    #Imax = exp(E_I*(Temp-t0_epsilon)/((k)*Temp*t0_epsilon))*epsilon1*Size^(epsilon2+cI*(Temp-t0_epsilon))*R^(epsilon3+cI*(Temp-t0_epsilon)) #saiz and calbet 2007
    #Imax = exp(E_I*(Temp-t0_Im)/((k)*Temp*t0_Im))*Im
    #Ingest = n/(1+(n/Imax)) #in units of micro grams of carbon per day
    Ingest = epsilon1*Size^(epsilon2)*R^(epsilon3)*Temp^(epsilon4)
