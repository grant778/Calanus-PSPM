...
DefaultParameters <- c(Delta =  1, #turnover rate is 1 divided by the per capita growth rate
                       ...
                       A_hat = 0.094, #liters per day filtering rate AKA volume swept clear via frost 1972. Units should be liters per day.
...
Modified_Parameters_A_hat_.096[3] = .096
...
output1_1_non_trivial_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
                                      ...
                                      stepsize = .1, parbnds = c(1, output1_1_A_hat_.096$bifpoints[1], 10000), parameters = Modified_Parameters_A_hat_.096, 
...
output1_1_non_trivial_.04 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
                                       biftype = "EQ", startpoint = c(output1_1_A_hat_.04$bifpoints[1], output1_1_A_hat_.04$bifpoints[2], 0), 
                                       stepsize = .1, parbnds = c(1, output1_1_A_hat_.04$bifpoints[1], 10000), 
...
output1_1_non_trivial_1.3344 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
                                        biftype = "EQ", startpoint = c(output1_1_A_hat_1.3344$bifpoints[1], output1_1_A_hat_1.3344$bifpoints[2], 0), 
                                        stepsize = .1, parbnds = c(1, output1_1_A_hat_1.3344$bifpoints[1], 10000), 
...
start_.096 = mean(which(t1[,"Rmax"] < (Modified_Parameters_A_hat_.096[2]+200) & t1[,"Rmax"] > (Modified_Parameters_A_hat_.096[2]-200)))
start_.04 = mean(which(t2[,"Rmax"] < (Modified_Parameters_A_hat_.04[2]+200) & t2[,"Rmax"] > (Modified_Parameters_A_hat_.04[2]-200)))
start_1.3344 = mean(which(t3[,"Rmax"] < (Modified_Parameters_A_hat_1.3344[2]+150) & t3[,"Rmax"] > (Modified_Parameters_A_hat_1.3344[2]-150)))
...
for(i in 1:nrow(DF3_J))
  {
  #print(which(abs(df3_J[,1] - DF3_J[,"Temperature"][i]) == min(abs(df3_J[,1] - DF3_J[,"Temperature"][i]))) )
  ...
  Year_to_temp_DF_J = df3_J[rows_to_extract_J,]
  Year_to_temp_DF_A = df3_A[rows_to_extract_A,]
...
for(i in 1:nrow(DF3_Summer))
  ...
  ggplot(data =  DF_Summer_Observed_Predicted, aes(x = Year, y = log(value), color = Data_Type, linetype = Stage))+
    #facet_wrap(~name)+
    geom_line()+
    geom_line()
