  ...
  
{
  ...
  Bering_SST = read.csv("Data/Bering_Sea_SST.csv")[c(49:65,67:70),] #1996 through 2012, 2014 through 2017
...
Diff_A = abs((log(Obs_A[,"value"])-log(Pred_A[,"value"]))/log(Obs_A[,"value"]))
Diff_J = abs((log(Obs_J[,"value"])-log(Pred_J[,"value"]))/log(Obs_J[,"value"]))
...
NLLJ_sum = c()
  ...
  {
NLLJ =  (log(Pred_J[start_J[i]:end_J[i], "value"])-log(Obs_J[start_J[i]:end_J[i], "value"]))^2/(2*(CVt_J)^2)
NLLJ_sum[i] = sum(NLLJ)
...
min(NLLJ_sum)
which(NLLJ_sum == min(NLLJ_sum)) #17th combination of parameters
...
for(i in 1:length(start_A))
{
  
  NLLA <- (log(Pred_A[start_A[i]:end_A[i], "value"])-log(Obs_A[start_A[i]:end_A[i], "value"]))^2/(2*(CVt_A)^2)
  NLLA_sum[i] <- sum(NLLA)
}
...
min(NLLA_sum)
which(NLLA_sum == min(NLLA_sum)) #16th combination of parameters


min_NLL = min(rowMeans(cbind(NLLJ,NLLA)))

which(rowMeans(cbind(NLLJ,NLLA)) == min_NLL) #17th combination of parameters
