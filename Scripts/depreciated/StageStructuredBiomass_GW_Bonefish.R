...
DefaultParameters <- c(Rho =  .1, #.1, #per day.  Range of between approximately .1 and 3 from Marañón et al. 2014.  They found no relationship between phytoplankton turnover rate and temperature  
                       ...
                       Temp = 282.15,  #20 C is 293.15 K (reference temp)
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
   ...
   #  Ingest        = switch(lifestage, Mc*Size*R/(Hc + R), q*Mc*Size*R/(Hc + R))
    ...
    netproduction = ((sigma*Ingest)/.095) - Metabolic_rate #need to convert ingestion parameter from micrograms of carbon to micrograms of dry weight of phytoplankton
