...
#
#                   The variable that is solved for is the bioSize of the resource.
#
...
#     2 : Total bioSize of juveniles
#     3 : Total bioSize of adults
#
...
#                     structured populations (such as number of bioSize of juveniles and
...
# bioSize. E[j] then can be expressed as E[j] = I[p][i]. Specify the
...
DefaultParameters <- c(Delta = 0.1, Rmax = 2.0, Sb = .01*(.0154*150^2.71), Sj = .0154*150^2.71, Sm = .0154*150^2.71, q = 1.0, Sigma = 0.5, T_ = 0.1, p = 1.0, Mu = 0.01,
                       A_hat = 3, 
...
LifeStageEndings <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    maturation  = switch(lifestage, Size - Sj, -1)
  })
}
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    #n = exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*(A_hat*((Size/Mopt)*exp(1-Size/Mopt))^(alpha))*R
    
    Imax =exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*epsilon1*Size^(epsilon2)
    
    #Metabolic_rate = exp(E_M*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*rho1*Size^rho2
    ...
    list(
      ...
      fecundity   = switch(lifestage, 0, (1-kappa)*netproduction/Sb),
      
      mortality   = Mu,
      
      impact      = switch(lifestage, c(Imax*(Size^q)*R, Size, 0), c(Imax*(Size^q)*R, 0, Size))
    )
  })
}
