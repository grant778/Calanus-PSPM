...
DefaultParameters <- c(Delta =  0.02, #turnover rate is 1 divided by the per capita growth rate
...
all_Adults = Bering_Calanus_Data[which(Bering_Calanus_Data[,"STAGE_NAME"] == "ADULT"), c(12,39)]
all_Juvenile = Bering_Calanus_Data[which(Bering_Calanus_Data[,"STAGE_NAME"] != "ADULT"), c(12,39)]
...
output1_1_non_trivial_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5_11_15_2023.R", 
                                      ...
                                      stepsize = 1.5, parbnds = c(1, output1_1_A_hat_.096$bifpoints[1], 10000), parameters = Modified_Parameters_A_hat_.096, 
...
#varied_temp = output1_1_non_trivial_varying_temperature_.096$curvepoints[,1]-273.15 #temp  
#mj =output1_1_non_trivial_varying_temperature_.096$curvepoints[,12]  #mj

mj = output1_1_non_trivial_varying_mj_.096$curvepoints[,1]
mj = round(mj, 2)
juvenile_biomass = as.numeric(output1_1_non_trivial_varying_mj_.096$curvepoints[,4])
adult_biomass = as.numeric(output1_1_non_trivial_varying_mj_.096$curvepoints[,5])
Total_biomass = 
...
a = ggplot(data = mj_df)  +
  ...
  geom_line(aes(group = 1, y = Biomass, x = mj)) +
  labs(x = "Size at Maturity (µg)", y = "Biomass Density (µg/L)") +
  theme_classic()

b = ggplot(data = mj_adult_juvenile_ratio)  +
  geom_line(aes(group = 1, y = Ratio, x = mj)) +
