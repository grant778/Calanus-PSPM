...
#
PSPMdimensions <- c(PopulationNr = 1, IStateDimension = 2, LifeHistoryStages = 2, ImpactDimension = 7)
...
DefaultParameters <- c(Delta =  0.08, #turnover rate is 1 divided by the per capita growth rate
                       ...
                       Rmax = 1620, #Rmax is a density micrograms of carbon per liter.  This means all other densities including copepod densities are micrograms per liter. Approximately 2000 from Putland and Iverson 2007
                       
                       A_hat = 0.098, #0.096 liters per day filtering rate AKA volume swept clear via frost 1972. Units should be liters per day.
                       ...
                       #Epsilon stuff is wrong as of 9/11/2023, used Ingestion in regression instead of Imax!!!!!!
                       epsilon1 = ,
                       ...
                       rho1 = 11.59214, #micro grams per day, from Ikeda_2007.  Dry weight at 2 C
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    n = exp(E_I*(Temp-t0)/((k)*Temp*t0))*A_hat*R #Remove Size impact from encounter rate and Mopt
    
    Imax = epsilon1*(Size^epsilon2)*(R^epsilon3)*exp(epsilon4*Temp)
    #Imax = exp(E_I*(Temp-t0_epsilon)/((k)*Temp*t0_epsilon))*epsilon1*Size^(epsilon2+cI*(Temp-t0_epsilon))
    ...
    list(
      ...
      mortality   = switch(lifestage, exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi)) , exp(E_mu*(Temp-t0_phi)/((k)*Temp*t0_phi))*phi1*Size^(phi2 + cI*(Temp-t0_phi))),  #Added temperature dependence to allometric exponent (Jan's paper doesn't include this)
      
      impact      = switch(lifestage, c(Ingest, Size, 0, netproduction, 0, Imax, Metabolic_rate), c(Ingest, 0, Size, 0, netproduction, Imax, Metabolic_rate))
    )
  })
}
