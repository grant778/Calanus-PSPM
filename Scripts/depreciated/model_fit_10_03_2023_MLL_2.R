...
EQ_Function = function(Delta, alpha, Rmax)
  
{
  ...
  return(log(DF_Summer_Observed_Predicted[which(DF_Summer_Observed_Predicted[,"Data_Type"] == "Predicted"),"value"])) #thousand µg
  
}

#t = EQ_Function(.01, .8, 47)
  
Nt = log(DF3[which(DF3[,"name"] == "Spring"),"value"])
  ...
  
{
 ...
 Rmax = par[3]
  ...
  Ntpred <- EQ_Function(Delta, alpha, Rmax)
...
z = optim(par=c(.058,.75,1100),fn=minimize.NLL, method="Nelder-Mead",Nt = Nt ,CVt = CVt, control = list(trace = TRUE))
