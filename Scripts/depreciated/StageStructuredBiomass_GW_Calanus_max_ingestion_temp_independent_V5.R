...
#
PSPMdimensions <- c(PopulationNr = 1, IStateDimension = 2, LifeHistoryStages = 2, ImpactDimension = 6)
...
DefaultParameters <- c(Rho =  .25, #.1, #per day.  Range of between approximately .1 and 3 from Marañón et al. 2014.  They found no relationship between phytoplankton turnover rate and temperature  
                       Rmax = 10000, #Rmax is a density micrograms of carbon per liter.  This means all other densities including copepod densities are micrograms per liter. Approximately 2000 from Putland and Iverson 2007
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    Ingest = n/(1+(n/Imax)) #in units of micro grams of carbon per liter per day
    ...
    list(
      ...
      impact      = switch(lifestage, c(Ingest, Size, 0, netproduction, 0, Imax), c(Ingest, 0, Size, 0, netproduction, Imax))
    )
  })
}
