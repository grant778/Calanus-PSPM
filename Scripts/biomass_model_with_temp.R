...
DefaultParameters <- c(Rho = 0.1, Rmax = 30.0, z = 0.01, Mc = 1.0, Hc = 3.0, q = 0.5, Sigma = 0.5, Tc = 0.1, Muc = 0.015, Muj = 0.0, Mua = 0.0,
                       Temp = 292.15, E_I = .45, T_0 = 292.15) # ml: adding temperature parameters
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    Ingest        = switch(lifestage, (Mc*exp(E_I*(Temp-T_0)/(0.0000862*Temp*T_0)))*Size*R/(Hc + R), q*(Mc*exp(E_I*(Temp-T_0)/(0.0000862*Temp*T_0)))*Size*R/(Hc + R)) # adding temp to mc (max intake)
