...
DefaultParameters <- c(Rho =  .25, #.1, #per day.  Range of between approximately .1 and 3 from Marañón et al. 2014.  They found no relationship between phytoplankton turnover rate and temperature  
                       ...
                       Temp = 273,  #20 C is 293.15 K (reference temp)
                       ...
                       phi1 = 0.123, #For mortality rate per day.  Approximated from  Hirst and Kiørboe 2002 calculated at 15 C.  Dry weight
                       ...
                       rho1 = 0.01607804, #micro grams per day, from Ikeda_2007.  Dry weight
                       rho2 = 0.852226  , #micro grams per day, from Ikeda_2007.  Dry weight
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
   ...
   #  Ingest        = switch(lifestage, Mc*Size*R/(Hc + R), q*Mc*Size*R/(Hc + R))
    ...
    Metabolic_rate = exp(E_M*(Temp-T_0)/((k)*Temp*T_0))*rho1*Size^(rho2 + cM*(Temp-T_0)) #parameters estimated from dry weight.  Units of micrograms of O2 per day
    
    
    
    netproduction = (sigma*(Ingest/.095)) - Metabolic_rate*.014196*(1/21)*(1000/1) #need to convert ingestion parameter from micrograms of carbon to micrograms of dry weight of phytoplankton with conversion factor of .095 Curl, Jr. H. 1962
