...
StateAtBirth <- function(E, pars)
{
  with(as.list(c(E, pars)),{
    ...
    c(Age = 0.0, Size = Egg) #Egg/C6 = z = #juvenile to adult ratio
  })
}
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    Imax = exp(E_I*(Temp-t0_epsilon)/((k)*Temp*t0_epsilon))*epsilon1*Size^(epsilon2+cI*(Temp-t0_epsilon)) #saiz and calbet 2011
