...
DefaultParameters <- c(Rho = .1, #?????
                       Rmax = 3, #?????? Rmax is a density micrograms per volume #Identify Rmax at which population persists
                       
                       A_hat = 5, #cant find information.  Unit of cubic centimeters per day or micrograms of carbon per individual per day?                                                          
                       Temp = 292.15,  #20 C is 293.15 K 19 degrees is 292.15 (reference temp)
                       E_mu = .85,  #.85 eV (Savage et al. 2004) 
                       E_M = .4, #.4 for C. glacialis (Tande 1988)
                       E_I = 0.46, #.46 for C. glacialis (Maps et al. 2012) 
                       E_delta = 0.43, 
                       ...
                       alpha = 0.75, #????
                       epsilon1 = 0.225, #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species.  Saiz and calbert found maximum ingestion rate was temp independent.
                       epsilon2 = 0.595, #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species.  Saiz and calbert found maximum ingestion rate was temp independent.
                       phi1 = 0.133, #Approximated from  Hirst and Kiørboe 2002 calculated at 15 C.  Dry weight
                       phi2 = -0.092, #Approximated from  Hirst and Kiørboe 2002 calculated at 15 C. Dry Weight.  Might need to change my reference temp?
                       rho1 = 0.13, #Approximated from  Mesocyclops brasilianus a tropical species taken at 22 C from Epp and Lewis 1980.  Dry weigh.  Ratio of dry to wet weight is  0.11 (Sitaramaiah 1967)
                       rho2 = 0.56, #Approximated from  Mesocyclops brasilianus a tropical species taken at 22 C from Epp and Lewis 1980.  Dry weight
                       ...
                       mj = 2*4^3.94,   # ug (micrograms) (Petersen, 1986) 
                       
                       z = 0.001591897 #juvenile to adult ratio 
                       )
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    n = exp(E_I*(Temp-T_0)/((k)*Temp*T_0))*(A_hat*((Size/Mopt)*exp(1-Size/Mopt))^(alpha+cI*(Temp-T_0)))*R #For both life stages n = ai*R
    
    Imax = exp(E_I*(Temp-T_0)/((k)*Temp*T_0))*epsilon1*Size^(epsilon2+cI*(Temp-T_0))
   ...
   #  Ingest        = switch(lifestage, Mc*Size*R/(Hc + R), q*Mc*Size*R/(Hc + R))
    ...
    Metabolic_rate = exp(E_M*(Temp-T_0)/((k)*Temp*T_0))*rho1*Size^(rho2 + cM*(Temp-T_0))
    
    
    
    netproduction = sigma*Ingest - Metabolic_rate
    ...
    list(
      ...
      mortality   = switch(lifestage, exp(E_mu*(Temp-T_0)/((k)*Temp*T_0))*phi1*Size^phi2, exp(E_mu*(Temp-T_0)/((k)*Temp*T_0))*phi1*Size^phi2),
