...
DefaultParameters <- c(Rho = 0.1, Rmax = 30.0, 
                       ...
                       epsilon1 = .25,
                       epsilon2 = .77,
                       phi1 = .0015, #Might be switched check literature phi and epsilon
                       phi2 = .75,
                       rho1 = .0123,
                       rho2 = .77,
                       mj = 12154.25, #.0154*150^2.71, #Size when zoop changes from juvenile to adult Length from Christine T. Baier, Jeffrey M. Napp 2003.  a and b parameters are from calanus finmarchius
                       mb = 121, #.01*(.0154*150^2.71), #Size of zoop at birth taken from value of z from de Roos et al 2008
                       z = (121)/(12154.25),
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    n = exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*(A_hat*((Size/Mopt)*exp(1-Size/Mopt))^(alpha))*R #For both life stages n = ai*R
