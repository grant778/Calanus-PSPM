...
DefaultParameters <- c(Delta = .1,
                       ...
                       sigma = .5,
                       T_0 = 295.15,
                       k = 8.617333262*10^(-5), #boltzman constant
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  ...
  with(as.list(c(E, pars, istate)),{
    list(
      ...
      development =  c(      1.0, Negative_to_zero(sigma*( ( ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R) /(1+( ( ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)) / ((exp(E_I*(Temp-T_0)/(k*Temp*T_0)))*epsilon1*Mass^(epsilon2)) )) - (exp(E_M*(Temp-T_0)/(k*Temp*T_0))*rho1*Mass^rho2) )  #sigma times ingestion              
      ...
      impact      = switch(lifestage, c(Imax*R/(R + Rh)*Mass^2, Mass, 0), #juveniles
                           
                           c(Imax*R/(R + Rh)*Mass^2, 0, Mass)) #adults  )  
