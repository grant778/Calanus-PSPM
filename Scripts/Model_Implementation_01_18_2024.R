...
#So year 2013 is missing zooplankton sample data
unique(Bering_Calanus_Data$YEAR)

all_Adults = Bering_Calanus_Data[which(Bering_Calanus_Data[,"STAGE_NAME"] == "ADULT"), c(12,33,42)]
all_Juvenile = Bering_Calanus_Data[which(Bering_Calanus_Data[,"STAGE_NAME"] != "ADULT"), c(12,33,42)]
...
all_calanus = data.frame(all_calanus) %>%
  mutate(Month = format(as.Date(all_calanus$GMT_DATE_TIME_TXT),"%m"), .after = GMT_DATE_TIME_TXT) 
...
#!!!Why were these years removed?
Bering_SST = read.csv("Data/Bering_SST_12_06.csv")[c(49:65,67:70),] #1996 through 2012, 2014 through 2017

#Leave this for now, check if the selected SST years match what is used for Biomass years
...
#It Looks like the years here match those selected above (1996:2012, 2014:2017), though I don't understand why 2017 is included... 
