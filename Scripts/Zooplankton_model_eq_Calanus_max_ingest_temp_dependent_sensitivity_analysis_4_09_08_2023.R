...
EQ_Function = function(Delta, Rmax, A_hat)
...
DefaultParameters <- c(Delta = Delta, #turnover rate is 1 divided by the per capita growth rate
                       ...
                       A_hat = A_hat, #.096 liters per day filtering rate AKA volume swept clear via frost 1972. Units should be liters per day.
                       ...
                       epsilon1 = exp(-8.95972),
                       epsilon2 = 0.35064, #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species. 15 C.
                       epsilon3 = 0.88576, #Approximated from saiz and calbert 2007.  micrograms of carbon per day.  On marine calanoid species.  15 C.
                       epsilon4 = 0.01603,  #Approximated from saiz and calbert 2007
                       t0_epsilon = 288.15,
...
output1_1_A_hat_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", biftype = "EQ", startpoint = c(0.05, 0.05), 
                     stepsize = 1.25,
                     parbnds = c(1, 1, 2000), parameters = Modified_Parameters_A_hat_.096, minvals = NULL, maxvals = NULL, options = c(c("popZE", "0")),
...
output1_1_non_trivial_.096 <-PSPMequi(modelname = "Scripts/StageStructuredBiomass_GW_Calanus_max_ingestion_temp_dependent_V5.R", 
                                      ...
                                      stepsize = 1, parbnds = c(1, output1_1_A_hat_.096$bifpoints[1], 2000), parameters = Modified_Parameters_A_hat_.096, 
...
start_.096 = median(which(t1[,"Rmax"] < (Modified_Parameters_A_hat_.096[2]+200) & t1[,"Rmax"] > (Modified_Parameters_A_hat_.096[2]-200)))
...
A_Hat_Vector = 0.098
alpha_vector = .75
Mopt_Vector = seq(150, 250, 10)
sensitivity_array = array(dim = c(length(Delta_Vector), length(Rmax_Vector), length(A_Hat_Vector)), 2) 
Pred_J = array(dim = c(0, 8, 8))
Obs_J =  array(dim = c(0, 8, 8))
Pred_A =  array(dim = c(0, 8, 8))
Obs_A =  array(dim = c(0, 8, 8))

Error_array_J = array(23, dim = c(length(Delta_Vector), length(Rmax_Vector), length(A_Hat_Vector)))
Error_array_A = array(21, dim = c(length(Delta_Vector), length(Rmax_Vector), length(A_Hat_Vector)))
...
for(i in 1:length(Delta_Vector))
{
  ...
  {
    for(k in 1:length(A_Hat_Vector))
        {
  cat("########################################")
  print(i)
  print(j)
  print(k)
  output = EQ_Function(Delta_Vector[i], Rmax_Vector[j], A_Hat_Vector[k])
  ...
  Obs_J = rbind(Obs_J, data.frame(cbind(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "J" ) ,],
                                        ...
                                        rep(A_Hat_Vector[k], nrow(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "J" ) ,]))
  )))
  ...
  Obs_A = rbind(Obs_A, data.frame(cbind(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "A" ) ,],
                ...
                rep(A_Hat_Vector[k], nrow(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "A" ) ,]))
  )))
  ...
  Pred_J = rbind(Pred_J, data.frame(cbind(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "J" ) ,],
                                        ...
                                        rep(A_Hat_Vector[k], nrow(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "J" ) ,]))
  )))
  ...
  Pred_A = rbind(Pred_A, data.frame(cbind(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "A" ) ,],
                                        ...
                                        rep(A_Hat_Vector[k], nrow(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "A" ) ,]))
  )))
...
for(i in 1:iterations)
{
  ...
  step = step+1
  Parameter_combo_mean_diff_A[i] = mean(Diff_A[(iter):(iter+length(years_A)-1)])
  Delta_A[i] = Obs_A[(iter),6]
  Rmax_A[i] = Obs_A[(iter),7]
  A_Hat_A[i] = Obs_A[(iter),8]
  print(iter)
  
  iter = iter + length(years_A)
...
for(i in 1:iterations)
{
  ...
  step = step+1
  Parameter_combo_mean_diff_J[i] = mean(Diff_J[(iter):(iter+length(years_J)-1)])
  Delta_J[i] = Obs_J[(iter),6]
  Rmax_J[i] = Obs_J[(iter),7]
  A_Hat_J[i] = Obs_J[(iter),8]
  print(iter)
  
  iter = iter + length(years_J)
...
which(rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J)) <= 1.5)
