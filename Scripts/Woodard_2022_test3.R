...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  ...
  with(as.list(c(E, pars, istate)),{
    ...
    development = c(      1.0, ((sigma*(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/(1+((((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*epsilon1*Mass^(epsilon2))))) - ((exp(E_M*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*rho1*Mass^rho2) )
      ...
      fecundity   = switch(lifestage, 0, ((sigma*(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/(1+((((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*epsilon1*Mass^(epsilon2))))) - ((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*rho1*Mass^rho2) )      
      ), #Net production
   
      mortality   = switch(lifestage, exp(E_mu*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2, exp(E_mu*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2),
      
      impact      = switch(lifestage, c(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/(1+(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R))/((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*epsilon1*Mass^(epsilon2))),
                                       Mass, #Juvenile biomass
                                        0), # Adult biomass
                          
                           c(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R)/(1+(((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R))/((exp(E_I*(Temp-T_0)/k*Temp*T_0)*phi1*Mass^phi2)*epsilon1*Mass^(epsilon2))),
                            0, #Juvenile biomass
                            Mass) #Adult biomass
      )  
      #Ingestion [0], juvenile biomass [1], adult biomass [2]
    )
  })
}
