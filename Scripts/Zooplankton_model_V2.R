...
Adult_Size_Vector = Ba0/sm #starting adult size vector (everything scaled to adult size of 1)
  ...
  
{
  ...
  if(juvenile_NBP < 0 )
  {
    ...
    #number vector is vector of counts in each size class
    #size vector is the size classes
    #lengths of size vector and number vector are equal
    
    for(j in 1:length(J1_Number_Vector))
    {
      
      for(a in 1:J1_Number_Vector[j])
    ...
    {
      Updated_J1_Number_Vector[j] = Updated_J1_Number_Vector[j]+1
    }
    else if(num<= dj)
    {
      Updated_J1_Number_Vector[j] = Updated_J1_Number_Vector[j]
    }
...
#q1 for adult metabolic rate q2 for adult temperature (relative to juveniles)