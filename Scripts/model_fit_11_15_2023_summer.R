...
EQ_Function = function(Delta, Rmax, A_hat, alpha, Mopt, sigma)
  
{
  ...
  DefaultParameters <- c(Delta =  Delta, #turnover rate is 1 divided by the per capita growth rate
                         ...
                         A_hat = A_hat, #0.096 liters per day filtering rate AKA volume swept clear via frost 1972. Units should be liters per day.
                         ...
                         mj = mj, #exp(-3.18)*exp(.73*12),   # ug (micrograms) (Petersen, 1986) pg 66
  ...
  start_.096 = median(which(t1[,"Rmax"] < (Modified_Parameters_A_hat_.096[2]+400) & t1[,"Rmax"] > (Modified_Parameters_A_hat_.096[2]-400)))
...
Delta_Vector = seq(.01,.04,.01) 

Rmax_Vector =  2000 #seq(35,50,1) #2000
#A_Hat_Vector = seq(0.035,.065,.01) 
A_Hat_Vector = 0.096

alpha_Vector = 0.75 #seq(0.63,0.72,0.01)

Mopt_Vector = seq(94,105,1)

sigma_Vector = 0.7 #seq(0.6,.63,.01) 
...
for(i in 1:length(Delta_Vector))
{
  ...
  {
    for(q in 1:length(A_Hat_Vector))
    {
  ...
  output = EQ_Function(Delta_Vector[i], Rmax_Vector[j], A_Hat_Vector[q], alpha_Vector[k], Mopt_Vector[z], sigma_Vector[a])
  ...
  Obs_J = rbind(Obs_J, data.frame(cbind(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "J" ) ,],
                                        ...
                                        rep(A_Hat_Vector[q], nrow(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "J" ) ,])),
  ...
  Obs_A = rbind(Obs_A, data.frame(cbind(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "A" ) ,],
                                        ...
                                        rep(A_Hat_Vector[q], nrow(output[which(output[,"Data_Type"] == "Observed" & output[,"Stage"] == "A" ) ,])),
  ...
  Pred_J = rbind(Pred_J, data.frame(cbind(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "J" ) ,],
                                        ...
                                        rep(A_Hat_Vector[q], nrow(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "J" ) ,])),
  ...
  Pred_A = rbind(Pred_A, data.frame(cbind(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "A" ) ,],
                                        ...
                                        rep(A_Hat_Vector[q], nrow(output[which(output[,"Data_Type"] == "Predicted" & output[,"Stage"] == "A" ) ,])),
...
for(i in 1:nrow(Pred_A))
if(Pred_A[i,"value"] < 0)
{
 Pred_A[i, "value"] = 0 
}

for(i in 1:nrow(Pred_J))
  if(Pred_J[i,"value"] < 0)
  {
    Pred_J[i, "value"] = 0 
  }

Obs_A[,"value"] = (Obs_A[,"value"] + 0.00000000001) #Add constant to deal with zeros and remove years without both adult and juvenile samples
Obs_J[,"value"] = (Obs_J[,"value"] + 0.00000000001)
Pred_A[,"value"] = (Pred_A[,"value"] + 0.0000000001)
Pred_J[,"value"] = (Pred_J[,"value"] + 0.0000000001)

CVt_J = sd(Obs_J[1:22,"value"],na.rm = TRUE)/mean(Obs_J[1:22,"value"], na.rm = TRUE)
CVt_A = sd(Obs_J[1:20,"value"],na.rm = TRUE)/mean(Obs_A[1:20,"value"], na.rm = TRUE)


Diff_A = (Obs_A[,"value"]-Pred_A[,"value"])^2 #squared residuals
Diff_J = (Obs_J[,"value"]-Pred_J[,"value"])^2
...
for(i in 1:iterations)
{
  ...
  Parameter_combo_mean_diff_A[i] = sum(Diff_A[start_A[i]:end_A[i]])
  Delta_A[i] = Obs_A[(start_A[i]),6]
  Rmax_A[i] = Obs_A[(start_A[i]),7]
  A_Hat_A[i] = Obs_A[(start_A[i]),8]
  alpha_A[i] = Obs_A[(start_A[i]),9]
  Mopt_A[i] = Obs_A[(start_A[i]),10]
  sigma_A[i] = Obs_A[(start_A[i]),11]
}
...
for(i in 1:length(start_J))
{
  ...
  Parameter_combo_mean_diff_J[i] = sum(Diff_J[start_J[i]:end_J[i]])
  Delta_J[i] = Obs_J[start_J[i],6]
  Rmax_J[i] = Obs_J[start_J[i],7]
  A_Hat_J[i] = Obs_J[start_J[i],8]
  alpha_J[i] = Obs_J[start_J[i],9]
  Mopt_J[i] = Obs_J[start_J[i],10]
  sigma_J[i] = Obs_J[start_J[i],11]
 
  
}

nrow(Obs_J)
#minimizing sum of squared residuals for adults and juveniles separately
min(Parameter_combo_mean_diff_J)
which(Parameter_combo_mean_diff_J == min(Parameter_combo_mean_diff_J)) #25
min(Parameter_combo_mean_diff_A)
which(Parameter_combo_mean_diff_A == min(Parameter_combo_mean_diff_A)) #21
...
which(rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J)) == min(rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J)))) #25
results = data.frame(cbind(Delta_J,Rmax_J,A_Hat_J,alpha_J,Mopt_J,sigma_J,rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J))))

results_ordered = results[order(rowMeans(cbind(Parameter_combo_mean_diff_A,Parameter_combo_mean_diff_J))),]
