#Indet_growth.R -  R file specifying the elementary life-history functions of
...
DefaultParameters <- c(Delta = 1, #De Roos 2008 Delta = 1
                       
                       Rmax = 2, #De Roos 2008 Rmax = 2
                       A_hat = 3, #guess                                                              
                       ...
                       z = (.01*(.0154*150^2.71))/(.0154*150^2.71))
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    n = switch(lifestage, exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R, 
               exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R*(A_hat*((Mass/Mopt)*exp(1-Mass/Mopt))^(alpha))*R) #For both life stages n = ai*R
    
    Imax = switch(lifestage,exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*epsilon1*Mass^(epsilon2), 
                  exp(E_I*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*epsilon1*Mass^(epsilon2))
    
    Metabolic_rate = switch(lifestage, exp(E_M*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*rho1*Mass^rho2,
    exp(E_M*(Temp-T_0)/((E_I/Temp)*Temp*T_0))*rho1*Mass^rho2)
    ...
    # Ingest        = Imax*(Size^q)*R
    # netproduction = Sigma*Ingest - T_*(Size^p)
    # len           = (Size - Sj)*(Size - Sj)/(Sm - Sj)
    # kappa         = switch(lifestage, 1.0, (1 - 3*len*len + 2*len*len*len))
    # 
    list(
      # We model a single structured population (nrow=1) with two i-state variables:
      development = c(1.0, Negative_to_zero(netproduction)),
      
      fecundity   = switch(lifestage, 0, Negative_to_zero(netproduction)/mb),
      ...
      impact      = switch(lifestage, c(Ingest, Mass, 0), c(Ingest, 0, Mass))
    )
  })
}
